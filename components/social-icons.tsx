...
const iconColors = {
  ...
  tiktok: 'text-black dark:text-white hover:text-gray-600 dark:hover:text-gray-300',
...
export function SocialIcon({ platform, size = 'md', className }: SocialIconProps) {
  ...
  const colorClass = iconColors[platform]
  ...
  return (
    <div className={cn('transition-colors duration-200', colorClass, className)}>
      {icons[platform]}
    </div>
  )
}
...
export function SocialLinks({ socialConnections, size = 'sm', showCount = false, className }: SocialLinksProps) {
  const platformMap = {
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      {socialConnections.map((connection, index) => {
        const platform = platformMap[connection.platform]
        if (!platform) return null

        return (
          <div key={index} className="flex items-center gap-1">
            <SocialIcon platform={platform} size={size} />
            {showCount && connection.followerCount && (
              <span className="text-xs text-muted-foreground">
                {formatFollowerCount(connection.followerCount)}
              </span>
            )}
          </div>
        )
